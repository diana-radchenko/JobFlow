...
const toggleSaved = (job: WorkJob & { saved: boolean }) => {
    if (job.saved) {
...
const salary = (job: WorkJob) => {
    ...
    const range =
        job.salary_start !== null && job.salary_end !== null
...
defineOptions({
    layout: {
        breadcrumbs: [{ title: 'Job Selection', href: jobSelectionRoute() }],
    },
});
</script>
...
<template>
    ...
    >
        ...
        >
            ...
            <div class="space-y-3">
                ...
                >
                    ...
                    <option
                        v-for="item in filterOptions.workplaceTypes"
                        :key="item"
                        :value="item"
                    >
                        {{ item }}
        ...
        <main class="min-w-0 flex-1 space-y-5">
            ...
            <div
                v-if="jobs.length === 0"
                class="rounded-2xl border border-dashed border-stone-300 px-6 py-16 text-center dark:border-stone-700"
            >
                <BriefcaseBusiness
                    class="mx-auto mb-4 h-10 w-10 text-stone-400"
                />
                <p class="font-semibold text-stone-700 dark:text-stone-200">
                    {{
                        activeView === 'saved'
                            ? 'No saved jobs yet'
                            : activeView === 'applied'
                ...
                <p class="mt-1 text-sm text-stone-500">
                    {{
                        activeView === 'saved'
                            ? 'Save interesting vacancies to review and compare them later.'
                            : activeView === 'applied'
                              ? 'Jobs you apply for will appear here so you can track their progress.'
                              : 'Try clearing one or more filters to see additional real vacancies.'
                    }}
                </p>
                <Button
                    v-if="activeView !== 'all'"
                ...
                >
                    <div class="min-w-0 space-y-4">
                        <div>
                            ...
                            <div class="mt-2 flex flex-wrap gap-2">
                                <Badge v-if="job.applied">Applied</Badge>
                                <Badge v-if="job.saved" variant="secondary"
                                    >Saved</Badge
                                >
                                <Badge
                        ...
                        >
                            ...
                            >
                            <span class="flex items-center gap-2"
                                ><CalendarDays class="h-4 w-4" />Posted
                                {{ posted(job) }}</span
                            >
                        </div>
                    </div>
                    ...
                    <div class="flex flex-wrap items-center justify-end gap-2 lg:flex-nowrap lg:self-center">
                        <Button
                            ...
                            variant="outline"
                            class="rounded-full"
                            @click.prevent.stop="toggleSaved(job)"
                            ><Bookmark class="mr-2 h-4 w-4" />{{
                                job.saved ? 'Saved' : 'Save'
                            }}</Button
                        >
                        <Button class="rounded-full px-8">{{
                            job.applied ? 'View application' : 'View vacancy'
                        }}</Button>
                    </div>
                </div>
            </Link>
