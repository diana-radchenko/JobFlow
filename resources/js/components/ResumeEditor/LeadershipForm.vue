...
import leadershipActivity from '@/routes/resume-editor/leadership-activity';
import type { LeadershipActivity } from '@/types/laravel-models';
...
const submit = () => {
    if (editingId.value) {
        form.put(
            leadershipActivity.update.url([props.resume.id, editingId.value]),
            { onSuccess: () => resetForm() },
        );
    } else {
        form.post(leadershipActivity.store.url(props.resume.id), {
            onSuccess: () => resetForm(),
        });
    }
};

const editItem = (item: LeadershipActivity) => {
...
const deleteItem = (id: number) => {
    if (confirm('Are you sure you want to delete this leadership activity?')) {
        useForm().delete(leadershipActivity.destroy.url([props.resume.id, id]));
    }
};

const toggleInclude = (item: LeadershipActivity) => {
    router.post(
        items.toggle.url([props.resume.id, 'leadership-activity', item.id]),
        {},
        { preserveScroll: true },
    );
};

const formatLocationLine = (item: LeadershipActivity): string =>
...
<template>
    <div class="space-y-6">
        <Card>
            <CardHeader>
                <CardTitle>Leadership &amp; Extracurricular</CardTitle>
                <CardDescription>
                    Choose which leadership roles and activities to include in
                    "{{ resume.title }}"
                </CardDescription>
            </CardHeader>
            <CardContent class="space-y-6">
                <!-- List of existing entries -->
                <div v-if="leadershipActivities.length > 0" class="space-y-4">
                    <div
                        v-for="item in leadershipActivities"
                        :key="item.id"
                    ...
                    <form @submit.prevent="submit" class="space-y-4">
                        ...
                        <div class="grid gap-2">
                            ...
                            <Textarea
                                ...
                                placeholder="Describe the role and what you achieved"
                                :rows="4"
                            />
                        ...
                        <div>
                            <Checkbox
                                ...
                                label="I currently hold this role"
                            />
                        ...
                        <div class="flex gap-3">
                            <Button type="submit" :disabled="form.processing">
                                ...
                                {{ editingId ? 'Update' : 'Add' }} Leadership
                                Activity
                            </Button>
                            <Button
                                type="button"
